...
"""
in addition i use maturin for building extension, so i need to add the extension to the root Cargo.toml file as a workspace member
...
members = [
    ...
    "packages/fabricatio-{{  cookiecutter.project_name }}",
]
"""
...
cargo_path: Path = Path.cwd().parent.parent / "Cargo.toml"

if not cargo_path.exists():
    print(f"Warning: {cargo_path} not found")
    exit(1)

# Parse the Cargo.toml file
cargo_doc = tomlkit.parse(cargo_path.read_text("utf-8"))

# Add to [workspace.members]
if "workspace" not in cargo_doc:
    cargo_doc["workspace"] = tomlkit.table()
if "members" not in cargo_doc["workspace"]:
    cargo_doc["workspace"]["members"] = tomlkit.array()

# Add the new project member
members_array = cargo_doc["workspace"]["members"]
new_member = f"packages/fabricatio-{project_name}"
if new_member not in members_array:
    members_array.append(new_member)

# Write the updated Cargo.toml
with open(cargo_path, "w", encoding="utf-8") as f:
    f.write(tomlkit.dumps(cargo_doc))

print(f"Successfully updated {cargo_path} with fabricatio-{project_name} workspace member")